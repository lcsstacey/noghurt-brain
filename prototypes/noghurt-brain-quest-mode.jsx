...
import { Lock, Check, X, AlertTriangle, Hash, Atom, Wifi, Globe, Gamepad2, Skull, Sparkles } from 'lucide-react';
...
// 6-node quest: 5 question nodes + 1 boss. The current node is index 2 (so we're "in progress").
const QUEST_NODES = [
  { biome: 'science',  done: true,  current: false },
  { biome: 'internet', done: true,  current: false },
  { biome: 'science',  done: false, current: true  }, // ← here
  { biome: 'retro',    done: false, current: false },
  { biome: 'internet', done: false, current: false },
  { biome: 'boss',     done: false, current: false }, // MAINFRAME
];

const CURRENT_NODE = QUEST_NODES.findIndex(n => n.current);
...
function PathNode({ node, index, total }) {
  const isBoss = node.biome === 'boss';
  const biome = isBoss ? null : BIOMES[node.biome];
  const Icon = isBoss ? Skull : biome.Icon;
  const color = isBoss ? C.red : biome.color;
  ...
  const stateStyles = {
    done:    { color, opacity: 0.4, background: '#000' },
    current: { color, opacity: 1,   background: `${color}11` },
    future:  { color: '#3f3f46', opacity: 0.7, background: '#000' },
  }[state];

  return (
    <div className="flex flex-col items-center gap-1.5 relative">
      {state === 'current' && (
        <div className="font-pixel text-[8px] absolute -top-5 whitespace-nowrap" style={{ color }}>
          ▼ HERE
        </div>
      )}
      <div className={`grid place-items-center border-2 transition-all ${state === 'current' ? 'node-pulse' : ''}`}
        style={{
          width: size, height: size,
          ...stateStyles,
          borderColor: stateStyles.color,
        }}>
        <Icon size={isBoss ? 26 : 20} style={{ color: stateStyles.color, filter: state !== 'future' ? `drop-shadow(0 0 4px ${stateStyles.color})` : 'none' }} />
        {state === 'done' && (
      ...
      <div className="font-pixel text-[8px]" style={{ color: state === 'current' ? color : '#52525b' }}>
        {isBoss ? 'BOSS' : `${(index + 1).toString().padStart(2, '0')}`}
      </div>
    </div>
  );
}
...
function PathLine({ active }) {
  return (
    <div className="flex-1 h-1 mx-1 self-center mt-2"
      style={{ color: active ? C.green : '#27272a' }}>
      ...
      <div className="relative flex items-center gap-1 sm:gap-2">
        {QUEST_NODES.map((node, i) => (
          <React.Fragment key={i}>
            <PathNode node={node} index={i} total={QUEST_NODES.length} />
            {i < QUEST_NODES.length - 1 && <PathLine active={node.done} />}
          </React.Fragment>
        ))}
      </div>
      <div className="relative mt-2.5 flex items-center justify-between font-pixel text-[8px] text-zinc-500">
        <span>QUEST PROGRESS · NODE <span style={{ color: C.cyan }}>{(CURRENT_NODE + 1).toString().padStart(2,'0')}</span> / {QUEST_NODES.length.toString().padStart(2,'0')}</span>
        <span className="flex items-center gap-1.5">
          <Skull size={9} style={{ color: C.red }} />
          <span style={{ color: C.red }}>MAINFRAME AHEAD</span>
        </span>
      </div>
    </div>
  );
      ...
      <div className="flex-1 min-w-0 relative">
        ...
        <div className="absolute -left-3 top-3 hidden sm:block">
          <svg width="12" height="12" viewBox="0 0 4 4" shapeRendering="crispEdges">
            <rect x="3" y="0" width="1" height="4" fill="#27272a" />
            <rect x="2" y="1" width="1" height="2" fill="#27272a" />
            <rect x="1" y="2" width="1" height="0" fill="#27272a" />
...
function PartyMember({ player, locked, size = 5 }) {
  return (
    <div className="flex flex-col items-center gap-1 min-w-[68px]">
      <div className="relative idle-bob" style={{ animationDelay: `${Math.random() * 0.6}s` }}>
